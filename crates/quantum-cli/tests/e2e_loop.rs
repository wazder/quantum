//! E2E: a counted loop in the guest. Proves intra-block Jcc rel works
//! end to end through the BlockTranslator.
//!
//! Guest program:
//!     mov  ecx, 10
//!     xor  r8d, r8d              ; sum = 0
...
//!     add  r8d, 7                ; sum += 7
//!     sub  ecx, 1
//!     jnz  .loop                 ; loop until ecx == 0
//!     mov  ecx, r8d              ; exit code = sum (70)
...
fn build_pe() -> Vec<u8> {
    ...
    // Data directory [1] IMPORT at RVA 0x2010, size 40
    let dd1 = opt + 112 + 8;
    ...
    // --- .text raw bytes ---
    //   RVA 0x1000  mov ecx, 10                 ; 5 bytes
    //   RVA 0x1005  xor r8d, r8d                ; 3 bytes (45 31 C0)
    // .loop @ RVA 0x1008
    //   RVA 0x1008  add r8d, 7                  ; 4 bytes (41 83 C0 07)
    //   RVA 0x100C  sub ecx, 1                  ; 3 bytes (83 E9 01)
    //   RVA 0x100F  jnz .loop                   ; 2 bytes (75 F7)
    //   RVA 0x1011  mov ecx, r8d                ; 3 bytes (44 89 C1)
    //   RVA 0x1014  call qword ptr [rip + disp] ; 6 bytes, target IAT @ 0x2000
    //                                              disp = 0x2000 - 0x101A = 0xFE6
    //   RVA 0x101A  ud2                          ; 2 bytes
    let text_raw = 0x400usize;
    let mut p = 0usize;
    // mov ecx, 10
    bytes[text_raw + p..text_raw + p + 5].copy_from_slice(&[0xB9, 0x0A, 0x00, 0x00, 0x00]);
    p += 5;
    // xor r8d, r8d
    bytes[text_raw + p..text_raw + p + 3].copy_from_slice(&[0x45, 0x31, 0xC0]);
    p += 3;
    // add r8d, 7
    bytes[text_raw + p..text_raw + p + 4].copy_from_slice(&[0x41, 0x83, 0xC0, 0x07]);
    p += 4;
    // sub ecx, 1
    bytes[text_raw + p..text_raw + p + 3].copy_from_slice(&[0x83, 0xE9, 0x01]);
    p += 3;
    // jnz .loop  (rel8 = -9 = 0xF7)
    bytes[text_raw + p..text_raw + p + 2].copy_from_slice(&[0x75, 0xF7]);
    p += 2;
    // mov ecx, r8d
    bytes[text_raw + p..text_raw + p + 3].copy_from_slice(&[0x44, 0x89, 0xC1]);
    p += 3;
    // call [rip + 0xFE6]
    bytes[text_raw + p..text_raw + p + 6].copy_from_slice(&[0xFF, 0x15, 0xE6, 0x0F, 0x00, 0x00]);
    p += 6;
    // ud2
    bytes[text_raw + p..text_raw + p + 2].copy_from_slice(&[0x0F, 0x0B]);

    // --- .idata at file 0x600 (RVA 0x2000) ---
    //   RVA 0x2000  IAT[0] -> hint/name @ RVA 0x2060
    //   RVA 0x2008  IAT terminator
    //   RVA 0x2010  IMAGE_IMPORT_DESCRIPTOR (20 bytes)
    //   RVA 0x2024  descriptor terminator (20 bytes)
    //   RVA 0x2040  ILT[0] -> hint/name @ RVA 0x2060
    //   RVA 0x2048  ILT terminator
    //   RVA 0x2060  hint/name for ExitProcess
    //   RVA 0x2080  DLL name
    let idata_raw = 0x600usize;
    bytes[idata_raw..idata_raw + 8].copy_from_slice(&0x2060u64.to_le_bytes());

    let desc = idata_raw + 0x10;
    bytes[desc..desc + 4].copy_from_slice(&0x2040u32.to_le_bytes()); // ILT
    bytes[desc + 12..desc + 16].copy_from_slice(&0x2080u32.to_le_bytes()); // Name
    bytes[desc + 16..desc + 20].copy_from_slice(&0x2000u32.to_le_bytes()); // FirstThunk
...
#[test]
fn loop_then_exit_uses_intra_block_branch() {
    ...
    let block = block::translate(&entry_bytes, entry_va, |op| matches!(op, Op::Ud2 | Op::Ret))
    ...
    // SAFETY: lifted block ends in CALL ExitProcess; ExitProcess longjmps out.
    ...
    // Loop iterates 10 times, each adding 7 → 70.
    assert_eq!(exit_code, 70, "expected loop body to run 10x and sum to 70");
}
